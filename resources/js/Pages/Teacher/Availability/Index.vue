    ...
    <AuthenticatedLayout>
        ...
        <div class="space-y-6">
            ...
            <div class="flex items-center justify-between">
                ...
                <button
                    @click="showForm = true"
                    class="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 text-sm font-medium focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
                >
            ...
            <div v-if="groupedSlots.length === 0" class="bg-white rounded-xl border border-gray-200 p-12 text-center">
                ...
                <p class="text-sm text-gray-500 mt-1">Click "Add Slot" to set your available time.</p>
            </div>
                    ...
                    <div class="divide-y">
                        <div v-for="slot in daySlots" :key="slot.id" class="flex items-center justify-between px-5 py-3">
                            ...
                            <button
                                ...
                                class="text-red-500 hover:text-red-700 text-sm font-medium focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
                            >
                                Delete
                            </button>
                        </div>
                ...
                <div class="bg-white rounded-xl shadow-xl p-6 w-full max-w-md mx-4">
                    ...
                    <form @submit.prevent="submit" class="space-y-4">
                        <div>
                            ...
                            <input type="date" v-model="form.slot_date" id="slot-date" class="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm" required />
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label for="slot-start-time" class="block text-sm font-medium text-gray-700 mb-1">Start Time</label>
                                <input type="time" v-model="form.start_time" id="slot-start-time" class="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm" required />
                            </div>
                            <div>
                                <label for="slot-end-time" class="block text-sm font-medium text-gray-700 mb-1">End Time</label>
                                <input type="time" v-model="form.end_time" id="slot-end-time" class="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm" required />
                            </div>
                        </div>
                        <p v-if="form.errors.slot_date" class="text-sm text-red-600">{{ form.errors.slot_date }}</p>
                        <div class="flex justify-end gap-3 pt-2">
                            <button type="button" @click="showForm = false" class="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">Cancel</button>
                            <button type="submit" :disabled="form.processing" class="px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 disabled:opacity-50 focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">Add Slot</button>
                        </div>
                    </form>
                </div>
            </div>
