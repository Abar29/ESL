...
watch(() => props.profile, (newProfile) => {
    if (newProfile) {
        ...
        form.value.gcash_name = newProfile.gcash_name || '';
        form.value.gotyme_number = newProfile.gotyme_number || '';
        form.value.gotyme_name = newProfile.gotyme_name || '';
        form.value.maya_number = newProfile.maya_number || '';
        form.value.maya_name = newProfile.maya_name || '';
        ...
        <div class="p-6 max-h-[80vh] overflow-y-auto">
            ...
            <div class="mb-6">
                ...
                <div class="flex items-center gap-4">
                    <div class="w-16 h-16 rounded-full bg-indigo-100 flex items-center justify-center flex-shrink-0 overflow-hidden">
                        <img v-if="profile?.profile_pic" :src="profile.profile_pic.startsWith('http') || profile.profile_pic.startsWith('data:') ? profile.profile_pic : '/storage/' + profile.profile_pic" class="w-16 h-16 rounded-full object-cover" alt="Profile picture" />
            ...
            <form @submit.prevent="submit" class="space-y-5">
                ...
                <div class="flex items-center gap-3 pt-4 border-t border-gray-100">
                    <button type="submit" :disabled="processing" class="px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 disabled:opacity-50 focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
                        Save Profile
                    </button>
                </div>
            </form>
